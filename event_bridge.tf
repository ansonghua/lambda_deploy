...
resource "aws_cloudwatch_event_target" "s3_object_create_invoke_justification_lambda" {
  rule           = aws_cloudwatch_event_rule.s3_object_create_invoke_jsutification_lambda.name
  arn            = aws_lambda_function.justification_lambda.arn
}
